...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	if (Status != XST_SUCCESS) {xil_printf("Error! Write failed!\r\n");}
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE writebuffer[1+nbytes];
	BYTE readbuffer[1+nbytes];
	writebuffer[0] = (reg + 0x2);
	for (int i = 0; i < nbytes; i++) {
		writebuffer[i+1] = data[i];
	}
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	Status = XSpi_Transfer(&SpiInstance, writebuffer, readbuffer, 1+nbytes);
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	if (Status != XST_SUCCESS) {xil_printf("Error! MaxbytesWrite failed!\r\n");}
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE writebuffer[] = {reg, 0};
	BYTE readbuffer[] = {0, 0};
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	Status = XSpi_Transfer(&SpiInstance, writebuffer, readbuffer, 2);
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	if (Status != XST_SUCCESS) {xil_printf("Error! Read failed!\r\n"); return 0;}
	return readbuffer[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
		...
		Status = XSpi_Transfer(&SpiInstance, writebuffer, readbuffer, 1+nbytes);
		XSpi_SetSlaveSelect(&SpiInstance, 0);
		if (Status != XST_SUCCESS) {xil_printf("Error! MaxbytesRead failed!\r\n");}
		else {
			for (int i = 0; i < nbytes; i++) {
